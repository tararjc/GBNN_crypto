...
with open(output_file, "w", newline="") as csvfile:
    ...
    for file in tqdm(csv_files):
        ...
        rmse_s = mean_squared_error(y_test.values, y_pred, squared=False)
        MAE_s = mean_absolute_error(y_test.values, y_pred)
        MSE_s = mean_squared_error(y_test.values, y_pred)
        ...
        r2_g = r2_score(y_test.values, pred_gbnn)
        writer.writerow(
        ...
        r2_mlp = r2_score(y_test.values, y_pred_MLP)
        writer.writerow(
            [extract_string(file, pattern), "MLP", rmse_mlp, MAE_mlp, MSE_mlp, r2_mlp]
        )

        data_gbnn = {"True": y_test, "Predicted": pred_gbnn}
        df_gbnn = pd.DataFrame(data_gbnn, index=y_test.index)

        # Create the second DataFrame
        data_sarima = {"True": y_test, "Predicted": pred_sarima}
        ...
        # Create a figure and two axis objects using Matplotlib
        ...
        fig.savefig(
            rf"results_close_price\{extract_string(file, pattern)}_Plot.png", dpi=700
        )
