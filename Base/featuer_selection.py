...
class FeatuerSelection_vol():
    def __init__(self, df):
        self.df = df

    def __call__(self):

        y = self.df['volatility']  # Target variable
        X = self.df.drop(['Close', 'volatility', 'Date'], axis=1)  # Drop non-numeric columns
        ...
        features = pd.DataFrame(selector.transform(
            x), columns=selected_feature_names)
        features = features.reset_index(drop=True)
        y = pd.DataFrame(y).reset_index(drop=True)
        date = self.df['Date'].reset_index(drop=True)
        df = pd.concat([date, features.join(y)], axis=1)
